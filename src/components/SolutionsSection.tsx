...
const SolutionsSection = () => {
  ...
  const handleMouseMove = (e: React.MouseEvent<HTMLDivElement>, index: number) => {
    ...
    if (holographic) {
      holographic.style.background = `radial-gradient(circle at ${x}px ${y}px, oklch(60.59% 0.213 292.72 / 0.12), oklch(55% 0.15 280 / 0.08), oklch(60% 0.12 220 / 0.06), transparent 50%)`;
    }
  };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8">
          {solutions.map((solution, index) => (
            <div
              ...
              className="group relative bg-card/50 backdrop-blur-sm border border-border rounded-lg overflow-hidden transition-all duration-300 md:hover:shadow-lg md:hover:shadow-primary/20 flex flex-col md:flex-row min-h-[280px]"
              style={{
            ...
            >
              {/* Content on Left */}
              <div
                className="relative z-10 py-8 px-8 space-y-4 flex-1 flex flex-col justify-start"
                style={{
                  transform: typeof window !== 'undefined' && hoveredIndex === index && window.innerWidth >= 1024 ? 'translateZ(25px)' : 'translateZ(0px)',
              ...
              <div
                className="holographic-overlay absolute inset-0 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none"
                style={{
                  background: 'radial-gradient(circle at 50% 50%, oklch(60.59% 0.213 292.72 / 0.12), oklch(55% 0.15 280 / 0.08), oklch(60% 0.12 220 / 0.06), transparent 50%)',
                  mixBlendMode: 'screen'
                }}
              />
            </div>
